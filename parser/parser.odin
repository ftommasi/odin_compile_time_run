...
parse_file :: proc(p: ^Parser, file: ^ast.File) -> bool {
	...
	if pkg_name.kind == .Ident {
		...
		case is_package_name_reserved(
			name,
		), file.pkg != nil && file.pkg.kind != .Runtime && name == "runtime":
...
fix_advance_to_next_stmt :: proc(p: ^Parser) {
	for {
		...
		case .Package, .Foreign, .Import, .If, .For, .When, .Return, .Switch, .Defer, .Using, .Break, .Continue, .Fallthrough, .Hash:
...
is_semicolon_optional_for_node :: proc(p: ^Parser, node: ^ast.Node) -> bool {
	...
	case ^ast.If_Stmt, ^ast.When_Stmt, ^ast.For_Stmt, ^ast.Range_Stmt, ^ast.Inline_Range_Stmt, ^ast.Switch_Stmt, ^ast.Type_Switch_Stmt:
		return true
...
parse_stmt :: proc(p: ^Parser) -> ^ast.Stmt {
	...
	case .Inline:
		...
		fallthrough
	// Operands
	case .No_Inline, .Context, .Proc, .Ident, .Integer, .Float, .Imag, .Rune, .String, .Open_Paren, .Pointer, .Asm, .Add, .Sub, .Xor, .Not, .And:
		// Also allows for 'context = '// Inline assembly
		// Unary Expressions
	...
	case .Hash:
		...
		//TODO: inspect cases here. We lift the defer parsing because that is very similar in terms of AST building, but there may be subtle differences
		case "run":
			tok := advance_token(p)
			stmt := parse_stmt(p)
			#partial switch s in stmt.derived_stmt {
			case ^ast.Empty_Stmt:
				error(p, s.pos, "empty statement after #run (e.g. ';')")
			case ^ast.Defer_Stmt:
				error(p, s.pos, "you cannot #run a defer statement")
				stmt = s.stmt
			case ^ast.Return_Stmt:
				error(p, s.pos, "you cannot #run a return statement")
			}
			cte_run := ast.new(ast.CTE_Run_Stmt, tok.pos, stmt)
			cte_run.stmt = stmt
			return cte_run
...
is_literal_type :: proc(expr: ^ast.Expr) -> bool {
	...
	case ^ast.Bad_Expr, ^ast.Ident, ^ast.Selector_Expr, ^ast.Array_Type, ^ast.Struct_Type, ^ast.Union_Type, ^ast.Enum_Type, ^ast.Dynamic_Array_Type, ^ast.Map_Type, ^ast.Bit_Set_Type, ^ast.Matrix_Type, ^ast.Call_Expr, ^ast.Bit_Field_Type:
		return true
	}
	return false
}
